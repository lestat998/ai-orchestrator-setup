...
5. Only `propose`, `spec`, `design`, `tasks`, `apply`, `verify`, and `archive` are single executable phase tokens. For one of those tokens, launch the allowed generic `sdd-executor` with that phase and its exact installed `skills/sdd-{phase}/SKILL.md` path. Carry structured status and `actionContext` into execution phases. Reject any unknown token as a contract error; never generate a skill name from it.
6. Treat `spec-and-design` as a parallel DAG layer, not a phase name. Launch one `sdd-executor` with phase `spec` and exact path `skills/sdd-spec/SKILL.md`, and one `sdd-executor` with phase `design` and exact path `skills/sdd-design/SKILL.md`; fan them out in parallel when supported and wait for BOTH envelopes. Never look for or launch `sdd-spec-and-design`.
...
CONTEXT:
- Delegate workspace resolution to the allowed generic executor; have it run `git rev-parse --show-toplevel 2>/dev/null || pwd`. The primary orchestrator must not run Bash.
