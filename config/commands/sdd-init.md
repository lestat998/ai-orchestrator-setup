...
You are the `ai-orchestrator-gpt`, not an SDD executor. Launch `sdd-executor-gpt` with phase `init` and the installed `sdd-init/SKILL.md` path only after preflight passes.
...
TASK:
Launch `sdd-executor-gpt` to detect the stack, conventions, architecture, testing capability, and strict TDD support. Persist project context as `sdd-init/{project}` and testing capabilities as `sdd/{project}/testing-capabilities`.
